...
class DLTEstimator(MotionEstimator):
    ...
    def add_new_landmarks(self, max_new=1000, reprojection_tol=10.0, min_baseline=0.1):
        ...
        # # rescale new points to match map's existing scale
        existing_depths = np.array([(self.P @ lm.pos)[2] for lm in self.landmarks])
        map_scale = np.median(existing_depths[existing_depths > 0]) if (existing_depths > 0).any() else None

        new_depths = np.array([(self.P @ pts3d_homo[:, i])[2] for i in ranked])
        new_scale  = np.median(new_depths[new_depths > 0]) if (new_depths > 0).any() else None

        if map_scale and new_scale and map_scale > 0 and new_scale > 0:
            pts3d_homo[:3] *= map_scale / new_scale

        for i in ranked:
            new_lm = Landmark(pts3d_homo[:, i].copy(), des_new[i].copy())
            self.landmarks.append(new_lm)
                
    def match_landmarks(self, img, prune_threshold=10, max_proj_dist=100):
        ...
        matched_ids = set(m.trainIdx for m in self.matches)
    ...
    def estimate(self, img, ransac=True):
        ...
        # opencv
        if True:
            pts3d = np.array([self.landmarks[m.trainIdx].pos[:3] for m in self.matches])
